...
def test_url_for(app):
    ...
    assert app.url_for("ItemsController.index") == "/api/items"
    assert app.url_for("ItemsController.create") == "/api/items"
    assert app.url_for("ItemsController.show", item_id=3) == "/api/items/3"
    assert (
        app.url_for("ItemsController.archive", year=2018, month=5) == "/api/items/2018/5"
    )
...
def test_url_for_object(app):
    ...
    assert app.url_for("ItemsController.archive", object) == "/api/items/2018/5"
...
def test_url_for_missing_param(app):
    ...
    with pytest.raises(MissingRouteParameter):
        app.url_for("ItemsController.archive", year="2018")
...
def test_url_for_bad_placeholder(app):
    ...
    with pytest.raises(BadRoutePlaceholder):
        app.url_for("ItemsController.archive", year=18, month=-3)
...
def test_url_for_extra_query(app):
    ...
    url = app.url_for("ItemsController.index", foo="bar")
    assert url == "/items?foo=bar"

    url = app.url_for("ItemsController.archive", year=2018, month=5, foo="bar")
