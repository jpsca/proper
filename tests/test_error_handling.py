...
# ═══════════════════════════════════════════════════════════════════
# Integration — do_request / run_pipeline error flow
# ═══════════════════════════════════════════════════════════════════


class TestDoRequestErrorFlow:
    def test_controller_exception_returns_500(self, app):
        app.router.add_route(
            Route(method="GET", path="/explode", to=_ExplodingController.index)
        )
        result = app.get("/explode")
        assert result.status == status.server_error

    def test_controller_http_error_preserves_status(self, app):
        app.router.add_route(
            Route(method="GET", path="/forbidden", to=_ForbiddenController.index)
        )
        result = app.get("/forbidden")
    ...
    def test_not_found_error_returns_404(self, app):
        ...
        result = app.get("/missing")
        assert result.status == status.not_found

    def test_unmatched_route_returns_404(self, app):
        result = app.get("/does-not-exist")
        assert result.status == status.not_found

    def test_error_stored_on_response(self, app):
        app.router.add_route(
            Route(method="GET", path="/explode", to=_ExplodingController.index)
        )
        current.app = app
        scope = make_test_scope("/explode", method="GET")
        response = app.do_test_request(scope)
        assert response.error is not None
        assert isinstance(response.error, ValueError)

    def test_on_error_called_on_controller_exception(self, app):
        called = []
    ...
    def test_on_teardown_called_on_controller_exception(self, app):
        called = []
    ...
    def test_on_teardown_called_on_success(self, app):
        called = []
        app.on_teardown(lambda: called.append("teardown"))
        app.router.add_route(
    ...
    def test_custom_error_handler_in_production(self, app):
        app.router.add_error_handler(ValueError, _ErrorPageController.handle)
        app.router.add_route(
            Route(method="GET", path="/explode", to=_ExplodingController.index)
        )
    ...
    def test_custom_error_handler_not_used_in_debug(self, app):
        app.debug = True
        app.router.add_error_handler(ValueError, _ErrorPageController.handle)
        app.router.add_route(
            Route(method="GET", path="/explode", to=_ExplodingController.index)
    ...
    def test_multiple_on_error_handlers_all_called(self, app):
        called = []
    ...
    def test_dbs_close_always_called_on_error(self, app):
        mock_db = MagicMock()
    ...
    def test_dbs_close_on_success(self, app):
        mock_db = MagicMock()
        mock_db.autoconnect = False
        mock_db.is_closed.return_value = False
        app.db = {"default": mock_db}
    ...
    def test_dbs_rollback_on_outer_error(self, app):
        """Rollback happens when run_pipeline itself propagates an error
    ...
    def test_dbs_skip_autoconnect(self, app):
    ...
    def test_dbs_skip_already_closed(self, app):
