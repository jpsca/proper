...
def test_getitem():
    """`md[key]` returns the last scalar value, mirroring `.get(key)`.
    Use `.getall(key)` when you need every value associated with the key.
    """
...
def test_setitem_appends():
    """Setting twice appends; `[]` reads the latest, `getall` reads all."""
    md = MultiDict()
    md["a"] = 1
    md["a"] = 2
    assert md["a"] == 2
...
def test_pop_returns_scalar():
    """`pop` (inherited from MutableMapping via __getitem__/__delitem__) must
    return the same scalar shape as `[]` and `.get()`. Regression: previously
    pop returned the underlying list, which silently broke any caller that
    ...
    md["k"] = "v2"
    assert md.pop("k") == "v2"
    assert "k" not in md  # pop removes the full key


def test_pop_default_when_missing():
    md = MultiDict()
    assert md.pop("missing", "fallback") == "fallback"
