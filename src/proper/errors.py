...
class BadSignature(ConfigError, itsdangerous.BadSignature):
    def __init__(self, message: str = "Bad signature", payload: t.Any = None):
...
class HTTPError(Exception):
    ...
    - status (int):
        HTTP status code, e.g. 500 or 725. If omitted, the class-level
        `status` attribute is used (subclasses set this to specialize,
        e.g. `class NotFound(HTTPError): status = 404`). An explicitly
        passed kwarg always wins over the class-level default.
...
class Forbidden(HTTPError):
    ...
    A server that wishes to hide the existence of a forbidden target
...
class MatchNotFound(NotFound):
    ...
    We couldn't find a matching route for the requested URL.

    This exception exists to help development by differentiating routing
    errors from other 404 Not Found exceptions caused by nonexistent
...
NotImplemented = HTTPNotImplemented  # noqa: A001
