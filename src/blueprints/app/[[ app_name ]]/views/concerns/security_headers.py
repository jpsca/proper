...
class SecurityHeaders:
    ...
    def after(self, view: View):
        ...
        view.response.headers.set("X-Frame-Options", "SAMEORIGIN")
        ...
        view.response.headers.set("X-XSS-Protection", "1", mode="block")

        # Download files or try to open them in the browser?
        view.response.headers.set("X-Download-Options", "noopen")

        # Set to none to restrict Adobe Flash Player’s access to the web page data.
        view.response.headers.set("X-Permitted-Cross-Domain-Policies", "none")

        view.response.headers.set("Referrer-Policy", "strict-origin-when-cross-origin")
