...
In this guide you will learn how to use Turbo from the server side of a Proper app. The idea behind Turbo is *HTML over the wire*: your Jx views render full pages, the contents of a frame, and individual stream fragments - all from the same templates, so you rarely write JavaScript.
...
Because Drive never does a full reload, the `<head>` from the first page sticks around. When you ship new CSS or JS and need the browser to pick it up, mark those tags with `data-turbo-track="reload"` and Turbo forces a full reload when they change:

```html+jinja
<link rel="stylesheet"
  href="{{ url_for('assets', file='css/app.css') }}"
  data-turbo-track="reload">
```

:::tip
`render_importmap()` already emits its tag with `data-turbo-track="reload"`, so a change to your import map busts the page automatically. You only need to add the attribute to your own tracked assets.
:::
...
When the form is invalid, the generated `validate_form` callback calls `self.redo()` - re-rendering `edit.jx` with a `422` status - so Turbo swaps the form, errors and all, back into the frame. Still no Turbo-specific code.
...
To make a frame's *own* navigations add browser history (so Back works), set `data-turbo-action="advance"` on the frame. `turbo_frame_tag` turns keyword underscores into dashes, so you pass it as `data_turbo_action`:
...
Argument                              | Renders
------------------------------------- | -------------------------------------------------
`component="path.jx"` (or positional) | the Jx component through the catalog, like `self.render`
...
To send several operations at once, concatenate fragments. They are `Markup`, so `+` (or passing a list to `render(stream=...)`) just works:
...
When a Turbo-driven form submits, Turbo adds `text/vnd.turbo-stream.html` to the `Accept` header. Proper registers that as the `turbo_stream` format, so the resolver routes the request to a `{action}.turbo_stream.jx` view automatically, falling back to the normal `.jx` template when there is not one:
