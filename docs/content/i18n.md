...
4. **`Accept-Language` header**<br>
  Begotiated against the locales you actually have catalogs for, via `app.i18n.negotiate_locale(request.accept_language)`. Good as a first guess.
...
That sounds harsh cookie set on a previous request. , but it's deliberate: a half-translated production page is worse than a clear error in development. You avoid the error two ways:
