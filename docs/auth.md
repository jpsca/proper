...
- Templates for sign-in, password reset, and email
...
# Validate a password reset token (returns user or None)
...
3. A `PasswordResetEmail` is sent asynchronously with a signed, time-limited token.
4. The user is redirected to a confirmation page.
5. When the user clicks the link in the email, the token is validated.
...
Password reset tokens are HMAC-signed strings containing:

- The user ID (base64-encoded)
- A timestamp (base36-encoded)
- An HMAC digest

The HMAC includes the user's password hash, so the token is automatically invalidated when the password changes. Tokens expire after `AUTH_TOKEN_LIFE` (default: 3 hours). All configured secret keys are tried when validating, supporting key rotation.
...
### 8.4 Customizing Error Messages
...
- **HMAC-signed** - tokens cannot be forged without the secret key
- **Time-limited** - tokens expire after a configurable lifetime
- **Password-bound** - changing the password invalidates all outstanding tokens
