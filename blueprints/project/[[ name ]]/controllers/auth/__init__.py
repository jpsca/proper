...
from [[ name ]].mailers import send_password_reset_email
...
class Auth(ApplicationController):
    ...
    def sign_in(self, req, resp):
        ...
        if not user:
            form.password.error = "We didn’t recognize that password."
            return
...
def go_forward(resp):
    next_url = resp.session.pop(REDIRECT_AFTER_LOGIN_KEY, None) \
        or app.url_for("Pages.index")
